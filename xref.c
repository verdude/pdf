...
void print_x_entry(x_entry_t* e) {
    log_v("%li %li %c", e->offset, e->gen, e->status);
}

void print_xref(xref_t* x) {
  log_v("%li %li", x->obj_num, x->count);
  log_v("Current entry offset: %li", x->ce_offset);
  log_v("Current entry index: %li", x->ce_index);
}
...
static int get_status(pdf_t* pdf) {
  if (!valid_xref_entry(pdf->xref)) {
    log_e("Invalid xref entry offset %li", pdf->xref->ce_offset);
    return -1;
  }
  ...
  switch ((unsigned char) c) {
    ...
    case 'n':
      return 1;
    default:
      log_e("Invalid xref entry char: [%c].", c);
      return -1;
  }
}
...
object_t* next_obj(pdf_t* pdf) {
  log_v("Getting xref entry #%li", pdf->xref->ce_index);
  int status;
  seek(pdf, pdf->xref->ce_offset, SEEK_SET);
  while (!(status = get_status(pdf))) {
    ...
    log_v("Skipping entry %li with invalid status: %i", pdf->xref->ce_offset, status);
  ...
  log_v("Getting next obj at %li", get_pos(pdf));
  return next_sym(pdf);
}
...
object_t* get_object(pdf_t* pdf, int obj_num) {
  ...
  if (obj_num < i || obj_num > x->obj_num + x->count) {
    log_e("Could not find obj #%i in the xref table", obj_num);
    scexit(pdf, 1);
  }
  ...
  if (status == -1 || !status) {
    log_e("Tried to read object with invalid status: %i", status);
    scexit(pdf, 1);
  }
  return next_obj(pdf);
}
...
int get_xref(pdf_t* pdf) {
  ...
  if (!(match = check_for_match(pdf, xref_string))) {
    log_e("Did not find xref table at offset: %li", offset);
    return 0;
  }
...
void parse_entries(pdf_t* pdf) {
  for (int i = pdf->xref->ce_index; i < pdf->xref->count; ++i) {
    ...
    log_v("Type Name: %s", get_type_name(o));
    free_object_t(o);
    checkout_next_obj(pdf);
  }
}
